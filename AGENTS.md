...
- **Backend dependency consistency.** Docker, backend PR checks, and pre-push use Python 3.14 and pip 26.2.1. `backend/requirements.txt` pins direct dependencies and loads `backend/constraints.txt` for indirect versions. Keep both files, the installer version, and CI cache inputs aligned. Do not bypass the shared pins with separate unbounded installations. `scripts/pre-push.sh` synchronizes these dependencies before formatting and testing.
