...
- **POST `/from-word`** — Rate limit: 30/min. Saves a single word as a flashcard: body `{word, context, cefr_level}`. Best-effort deduplication against the user's flashcards on the active plan using a targeted case-insensitive SQL match (`lower(trim(word))`, oldest card wins): if the input word already exists, the existing card is returned without calling the AI; otherwise AI generates definition/example/translation (which may canonicalize the word to a different lemma), and the canonicalized word is checked the same way before inserting. Only inserts (`source="from_text"`) when neither check matches; two concurrent first-time saves of the same word can still both insert. Returns `FlashcardFromWordResponse` (`FlashcardResponse` plus `already_saved: bool`). `already_saved` means the word was already visible in My Vocabulary (`source="from_text"`); a matching generated or imported card (`source` NULL) is promoted to `source="from_text"` in place and returned with `already_saved=false`, so it appears in the vocabulary list without creating a duplicate
